...
# More settings: http://matplotlib.org/api/axes_api.html#matplotlib.axes.Axes.plot 
marker='o'
markersize=5
linestyle= '' #'dotted' # 'solid'   'dashed'   'None'
alpha=.7
markeredgecolor='none' 
patch_edgecolor='none' # None (with capital N) results in matplotlib reverting to Default


SF_xmin, SF_xmax, SF_ymin, SF_ymax = 0.5, 10001, 0.5, 10001
ER_xmin, ER_xmax, ER_ymin, ER_ymax = 0.5, 20, 0.5, 2500
axis_limits = {'log':[[SF_xmin, SF_xmax],[SF_ymin, SF_ymax]], 'linear':[[ER_xmin, ER_xmax],[ER_ymin, ER_ymax]]}
...
def getCommandLineArgs():
    if len(sys.argv) < 4:
        print ("Usage: python3 degree_dist.py   [/absolute/path/to/input_file.txt]   [x-axis scale: log, linear, logit, or symlog]   [y-axis scale]\nExiting..\n")
        sys.exit()
...
if __name__ == "__main__":
    input_file,  xscale, yscale = getCommandLineArgs()
    plots_dir=os.path.join(os.getcwd(), input_file.split('/')[-1].split('.')[0])
    if not os.path.isdir(plots_dir):
        os.mkdir(plots_dir)
    colors = get_colors()
    
    palettes  = [['BuGn',    'Set2',          'OrRd',      'green',  'RdBu',            'coolwarm', 'bright',     'black'], 
                 ['RdYlGn',  'sns_cubehelix', 'RdBu',      'GnBu',   'stack2',          'PuBu',     'bright',     'pastel'],
                 ['RdYlBu',  'RdPu_r',        'muted',     'dark',   'sns_hls_palette', 'Set2',     'BuGn',       'PuOr'],
                 ['Paired',  'YlOrBr',        'husl',      'YlGnBu', 'RdGy',            'green',    'colorblind', 'BrBG'],
                 ['PiYG',    'black',         'cubehelix', 'stack',  'flatui',          'red',      'Spectral',   'YlGn' ]]
    
    intensity = [5]     # color 0: faint, 5:intense
    H_in, H_ou = [], []
    init =0
    for p in [0]: #range(len(palettes)):
        ...
        for i in intensity:
            ...
            indeg_fig = fig.add_subplot (211) #The Axes instance will be returned by fig.add_subplot()
            oudeg_fig = fig.add_subplot (212)
            

            #indeg_fig.set_autoscalex_on(False) 
            
            
            indeg_fig.set_xscale(xscale)
            indeg_fig.set_yscale(yscale)
            indeg_fig.set_xlim(axis_limits[xscale][0])
            indeg_fig.set_ylim(axis_limits[yscale][1])
            
              
            oudeg_fig.set_xscale(xscale)
            oudeg_fig.set_yscale(yscale)
            oudeg_fig.set_xlim(axis_limits[xscale][0])
            oudeg_fig.set_ylim(axis_limits[yscale][1])


            
            in_patch, out_patch, H_in, H_ou, j = [], [] , [], [], 0
            for network_file in open(input_file,'r').readlines():
                
                M = load_network (network_file.strip())
                if init==0:                   
                    print ((network_file.split('/')[-1].strip()).ljust(22,' ')+"nodes "+str(len(M.nodes())).ljust(7,' ')+"edges "+str(len(M.edges())).ljust(10,' '))
                ...
                indeg_fig.plot(indegs, indegs_frequencies, linestyle=linestyle,  color = colors[palettes[p][j]][i], alpha=alpha,
                                markersize=markersize, marker=marker, markeredgecolor=markeredgecolor)
            
                tmp = itemfreq(ou_degrees)
                outdegs, outdegs_frequencies = tmp[:, 0], tmp[:, 1] 
                oudeg_fig.plot(outdegs, outdegs_frequencies,linestyle=linestyle, color = colors[palettes[p][j]][i], alpha=alpha,
                               markersize=markersize, marker=marker, markeredgecolor=markeredgecolor)
         
                in_patch =  mpatches.Patch(color=colors[palettes[p][j]][i], label=(network_file.split('/')[-1]).split('.')[0])
                out_patch = mpatches.Patch(color=colors[palettes[p][j]][i], label=(network_file.split('/')[-1]).split('.')[0]) #, edgecolor=patch_edgecolor, alpha=alpha
            ...
            indeg_fig.tick_params(axis='both', which='both', right='off', top='off',  pad=3, width=.5,  length=2) #http://matplotlib.org/api/axes_api.html#matplotlib.axes.Axes.tick_params                  
            oudeg_fig.tick_params(axis='both', which='both', right='off', top='off',  pad=3, width=.5,  length=2) #http://matplotlib.org/api/axes_api.html#matplotlib.axes.Axes.tick_params
    
            indeg_fig.legend(handles=H_in, frameon=False,fontsize= 6, shadow=False, loc='upper right',  bbox_to_anchor = (0.9, 0.7))
            indeg_fig.set_xlabel('Degree (log) ')
            if xscale == 'log':  
                indeg_fig.set_xlabel('Degree (log)')
                oudeg_fig.set_xlabel('Degree (log)')
            else:
                indeg_fig.set_xlabel('Degree')
                oudeg_fig.set_xlabel('Degree')
            if yscale == 'log':
                indeg_fig.set_ylabel('Number of nodes (log)')
                oudeg_fig.set_ylabel('Number of nodes (log)')
            else:
            ...
            plt.savefig(slash(plots_dir)+plots_dir.split('/')[-1].split('.')[0]+"_in-out-deg-dist-"+str(p+1)+"-"+str(i+1)+".png", dpi=400) # http://matplotlib.org/api/figure_api.html#matplotlib.figure.Figure.savefig   
    
            print ("Palette #"+str(p+1)+" intensity "+str(i+1)+", written: "+slash(plots_dir.split('/')[-1].split('.')[0])+plots_dir.split('/')[-1].split('.')[0]+"_in-out-deg-dist-"+str(p+1)+"-"+str(i+1)+".png")
    
    #oudeg_fig.set_title('Out-degree Distribution')
    #indeg_fig.set_title('In-degree Distribution')    
    # autoscale will force xlim and ylim to be (0:..)
    #indeg_fig.autoscale(enable=True, axis='both')
    #oudeg_fig.autoscale(enable=True, axis='both')
     
    #xmin, xmax, ymin, ymax = indeg_fig.axis()   
    #print ("xmin "+str(xmin)+", xmax"+" "+str(xmax)+", ymin"+" "+str(ymin)+", ymax"+" "+str(ymax))

    #indeg_fig.semilogx(indegs, indegs_frequencies, basex=10, linestyle='-',  color = colors[palette[i]][5], alpha=0.7, markersize=7, marker='', markeredgecolor=colors[palette[i]][5])
    #indeg_fig.semilogy(indegs, indegs_frequencies, basey=10, linestyle='-',  color = colors[palette[i]][5], alpha=0.7, markersize=7, marker='', markeredgecolor=colors[palette[i]][5])                                 
    
    #indeg_fig.get_xaxis().get_major_formatter().labelOnlyBase = False
    #indeg_fig.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    #indeg_fig.set_autoscaley_on(False) 
    #indeg_fig.set_ylim([-10,10000])
    #indeg_fig.set_autoscalex_on(False) 
    #indeg_fig.set_xlim([-10,10000])
            
    
            #indeg_fig.set_xticks([1, 5, 10,100,1000,10000])
            #indeg_fig.set_yticks([1, 5, 10,100,1000,100000])    
            #oudeg_fig.set_xticks([1, 5, 10,100,1000,10000])
            #oudeg_fig.set_yticks([1, 5, 10,100,1000,100000])